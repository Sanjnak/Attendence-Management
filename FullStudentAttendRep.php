...
$entry = mysqli_query($conn, "SHOW COLUMNS FROM attendencetable");
...
<?php
          ...
          ?>
                <tr class="border-b  odd:bg-gray-100 even:bg-gray-300">
                  <td class="px-56 py-4 font-medium text-black whitespace-nowrap"><?php echo $row->Field; ?></td>
                  <td class="px-56 py-4 font-medium text-black whitespace-nowrap"><?php echo $result1[$i]; ?></td>
                </tr>
          <?php
              $i++;
              }
...
<div class=" text-base font-bold text-cyan-700 font-family: 'Open Sans', sans-serif; ml-12 mt-5">
<h5>Total Days :
    <?php 
    $i = (sizeof($result1)/2) - 2;
    echo $i; 
    ?>
   </h5><br>
  <h5>Present  : 
...
<div class="flex items-center justify-between">
<button onClick="printPageArea('main')" class="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-6 ml-12 mt-8 rounded focus:outline-none focus:shadow-outline">Print the report</button>
    </div>
  </div>
</body>
